...
// the base "surface shader" for the Nagai demo. designed to be as versatile as possible for solid, opaque surfaces.

Shader "GraphicsLearning/NagaiSurfaceV3"
{
    Properties
    {
    ...
    CGINCLUDE
        ...
        // these two are defined/used in UnityCG.inc, redefining them causes problems since
        // it's included via the NagaiLighting cginc.
        // sampler2D _MainTex;
        // float4 _MainTex_ST;
        ...
        // global texture for triplanar noise. set this via script.
        sampler2D _NagaiGlobalNoise;

        // Unity magic variables for lightmaps. make sure to use these names or it won't work.
        // These are currently defined in some include somewhere, so we're not redefining them.
        ...
        struct appdata
        {
            // stick with the Unity naming conventions in this struct -
        ...
        struct v2f
        {
            ...
            float2 lightmap : TEXCOORD6;
            // this packs light coords into TEXCOORD7
            // and shadow coords into TEXCOORD8
        ...
        v2f vert (appdata v)
        {
            ...
            // make sure to offset lightmap coords by scale and tiling
            o.lightmap = v.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
            
            // at one point, we did full SH9 spherical harmonics ambient light calculation...
            // ...but this shader became so stylized, simply referencing the built in unity_AmbientSky,
            // which gets calculated via an average from the skybox automatically, is good enough.
            
            //o.sh= ShadeSH9 (half4(normalWorld, 1));
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // only difference here from the unity fallback 
            // is the use of clip to support alpha cutouts in shadows.
            Name "ShadowCaster"
